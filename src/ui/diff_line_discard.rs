...
pub(crate) fn apply_selected_lines_to_base_text(
    file_path: &str,
    diff_text: &str,
    base_text: &str,
    selections: &HashSet<DiffLineSelection>,
    ...
    if selections.is_empty() {
        return Err("Select at least one changed line to commit.".to_string());
    }

    let edits = selected_line_edits_for_base(file_path, diff_text, selections);
    if edits.is_empty() {
        return Err("No selected changed lines are present in the current diff.".to_string());
    }

    let had_final_newline = base_text.ends_with('\n');
    let mut lines: Vec<String> = base_text.lines().map(ToString::to_string).collect();
    let mut edits = edits;
    edits.sort_by(compare_edits_for_application);
...
fn selected_line_edits_for_base(
    file_path: &str,
    diff_text: &str,
    selections: &HashSet<DiffLineSelection>,
) -> Vec<TextEdit> {
    ...
    for raw_line in diff_text.lines() {
        ...
        if let Some(content) = raw_line.strip_prefix('+') {
            ...
            if selections.contains(&target) {
                edits.push(TextEdit::Insert {
                    index: old_line.saturating_sub(1),
                    content: content.to_string(),
                    sequence,
                });
            }
...
mod tests {
    ...
    #[test]
    fn applies_selected_added_line_to_base() {
        let diff = "@@ -1,2 +1,3 @@\n one\n+two\n three";
        let mut selected = HashSet::new();
        selected.insert(target("code.txt", DiffLineSelectionKind::Added, 2));

        let next =
            apply_selected_lines_to_base_text("code.txt", diff, "one\nthree\n", &selected).unwrap();
        assert_eq!(next, "one\ntwo\nthree\n");
    }

    #[test]
    fn applies_selected_deleted_line_to_base() {
        let diff = "@@ -1,3 +1,2 @@\n one\n-two\n three";
        let mut selected = HashSet::new();
        selected.insert(target("code.txt", DiffLineSelectionKind::Deleted, 2));
    ...
    #[test]
    fn applies_selected_replacement_pair_to_base() {
        let diff = "@@ -1,3 +1,3 @@\n one\n-two\n+two changed\n three";
        let mut selected = HashSet::new();
        selected.insert(target("code.txt", DiffLineSelectionKind::Deleted, 2));
        selected.insert(target("code.txt", DiffLineSelectionKind::Added, 2));
