...
pub(crate) fn render_changes_context_menu_overlay(
    ...
    // .occlude() is the fix for the hover bleed-through this menu used to
    // have: GPUI only stops a hitbox behind this one from reporting
    // "hovered" if something in front explicitly blocks it — being painted
    // on top is not enough on its own. Without this, hovering a menu item
    // also lit up whatever file-list row sat underneath it on screen.
    Some(
        div()
